package speedy.comparison.generator;
...
public class ComparisonScenarioGenerator {
    ...
    private NullValue getNextPlaceholder() {
        lastPlaceholderId++;
        return new NullValue(ComparisonConfiguration.getStringSkolemPrefixes()[0] + lastPlaceholderId);
    }
    ...
    private IValue generateNewValue(String type) {
        if (random.nextBoolean()) { //New Constant
            ...
            return new NullValue(ComparisonConfiguration.getStringSkolemPrefixes()[0] + randomPlaceholderId);
        }
    }
